...
int hh_v20_dispatch(FILE *stream_in, FILE *stream_out, pq_header_t *pq_header, 
	...
	result = hh_v20_header_read(stream_in, &hh_header);
	if ( hh_header == NULL ) {
	...
	if ( hh_header->MeasurementMode == HH_MODE_INTERACTIVE ) {
		result = hh_v20_interactive_stream(stream_in, stream_out, pq_header, 
				hh_header, options);
	} else if ( hh_header->MeasurementMode == HH_MODE_T2  || 
				hh_header->MeasurementMode == HH_MODE_T3 ) {
		result = hh_v20_tttr_stream(stream_in, stream_out, pq_header,
				hh_header, options);
	} else {
...
hh_v20_header_t *hh_v20_header_alloc(int input_channels) {
	...
	if ( hh_header != NULL ) {
		hh_header->InpChan = (hh_v20_input_channel_t *)malloc(
				sizeof(hh_v20_input_channel_t)*input_channels);
	} 
 ...
 *
 */
void hh_v20_header_printf(FILE *stream_out, 
		hh_v20_header_t *hh_header) {
	int i;
